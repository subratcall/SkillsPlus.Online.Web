            ...
            <div class="col-lg-12">
                ...
                <form id="form" class="form-horizontal form-bordered" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-10">
                            <input type="text" name="title" id="title" class="form-control" readonly value="I want to be an instructor">
                        </div>
                    </div>
...
var id = "{{request()->route('id')}}";
    ...
    function categories() {
        $.ajax({
            url: "{{ url('/admin/user_vendor/getAllCategory') }}",
            type: "get",
            dataType: 'JSON',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            ...
            success: function(data) {
                ...
                data.data.forEach(function(entry) {
                    $("#category_id").append("<option value='" + entry.id + "' selected>" + entry.title + "</option>")
                });
            },
            error: function(jqXHR, textStatus, errorThrown) {
                alert('Error! Contact IT Department.');
    ...
    var data = $('#form').serializeArray();
        $.ajax({
            url:  "{{ url('/admin/user_vendor/vendor_request') }}",
            type: "post",
            data: data,
            dataType: 'JSON',
            success: function(data) {
