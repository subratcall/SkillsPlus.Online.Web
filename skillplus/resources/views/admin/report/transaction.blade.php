            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="input-group">
                        <input type="date" id="fsdate" value="{!! $_GET['fsdate'] or '' !!}"
                            class="text-center form-control" name="fsdate" placeholder="Start Date">
                ...
                <div class="col-md-4">
                    <div class="input-group">
                        <input type="date" id="lsdate" value="{!! $_GET['lsdate'] or '' !!}"
                            class="text-center form-control" name="lsdate" placeholder="End Date">
    ...
    <div class="card-body">
        <table class="table table-bordered table-striped mb-none display responsive nowrap" cellspacing="0" width="100%"
            ...
            <tbody>
                ...
                <tr>
                    <td class="text-center"><a href="/profile/{{{ $item->buyer->id or 0 }}}"
                            target="_blank">{{{ $item->buyer->name or '' }}}</a></td>
                    <td class="text-center"><a href="/profile/{{{ $item->user->id or 0 }}}"
                            target="_blank">{{{ $item->user->name or '' }}}</a></td>
                    <td class="text-center"><a href="/product/{{{ $item->content->id or 0 }}}"
                            target="_blank">{{{ $item->content->title or '' }}}</a></td>
                    <td class="text-center">{{{ $item->price or 0 }}} {{{  trans('admin.cur_dollar') }}}</td>
                    <td class="text-center">{{{ $item->income or 0 }}} {{{  trans('admin.cur_dollar') }}}</td>
