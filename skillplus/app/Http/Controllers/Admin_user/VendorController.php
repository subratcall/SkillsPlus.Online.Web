...
class VendorController extends Controller
{
    ...
    public function saveCourseLearn(Request $request)
    {
        if ($request->mode == "Save") {
            ContentLearn::create([
                ...
                'description' => $request->title_learn,
            ]);            
        } else if ($request->mode == "Update") {
            ContentLearn::where(['id' => $request->learn_id])->update([
                'content_id' => $request->content_id,
                'description' => $request->title_learn,
            ]);
        }
    ...
    public function saveCourseRequiremnts(Request $request)
    {
        if ($request->mode == "Save") {
            ContentRequirements::create([
                'content_id' => $request->content_id,
                'requirement' => $request->title_req,
            ]);            
        } else if ($request->mode == "Update") {
            ContentRequirements::where(['id' => $request->req_id])->update([
                'content_id' => $request->content_id,
                'requirement' => $request->title_req,
            ]);
        }
    ...
    {
        $data = ContentRequirements::where('id', $id)->first();
        echo json_encode($data);
    }

    public function destroyCourseRequirements($id)
    {
        ContentRequirements::where('id', $id)->delete();
        echo true;
    }
}
