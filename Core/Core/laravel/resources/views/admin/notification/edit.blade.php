                ...
                <div class="form-group">
                    ...
                    <div class="col-md-11" >
                        <select  name="recipent_type" class="form-control populate recipent_selection">
                                <option value=""></option>
                                <option value="user" @if($item->recipent_type == 'user') {{{ 'selected' }}}@endif >{{{ trans('admin.single_user') }}}</option>
                    ...
                    <div class="col-md-11" dir="ltr">
                        <select name="recipent_list_users[]" multiple data-plugin-selectTwo class="form-control populate text-left">
                            @foreach($users as $user)
                                <option value="{{{ $user->id }}}" @if($item->recipent_type == 'users' && in_array($user->id,unserialize($item->recipent_list))) {{{ 'selected' }}} @endif>{{{ $user->username }}} ({{{ $user->name }}})</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group recipent" id="user" @if($item->recipent_type == 'user') {!! 'style="display: block;"' !!} @endif>
                    ...
                    <div class="col-md-11">
                        <select name="recipent_list_user" class="form-control populate">
                            @foreach($users as $user)
                                <option value="{{{ $user->id }}}" @if($item->recipent_type == 'user' && $item->recipent_list == $user->id) {{{ 'selected' }}} @endif >{{{ $user->username }}} ({{{ $user->name }}})</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-md-11">
                        <select name="recipent_list_category" class="form-control populate">
                            @foreach($userCategory as $cat)
                                <option value="{{{ $cat->id }}}" @if($item->recipent_type == 'category' && $item->recipent_list == $cat->id) {{{ 'selected' }}} @endif >{{{ $cat->title }}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
