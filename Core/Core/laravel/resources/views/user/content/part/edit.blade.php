@extends('user.layout.partlayout')
@section('tab1','active')
@section('tab')
    <div class="row">
        <div class="h-20"></div>
        <div class="col-md-7 col-xs-12">
            <div class="ucp-section-box">
                ...
                <div class="body">
                    <form action="/user/content/part/edit/store/{{{ $part->id or '' }}}" method="post">

                        <input type="hidden" name="content_id" value="{{{ $part->content->id or '' }}}">

                        <div class="form-group">
                            <label class="control-label" for="inputDefault">{{{ trans('main.title') }}}</label>
                            <input type="text" name="title" value="{{{ $part->title or '' }}}" class="form-control" required>
                        </div>

                        <div class="form-group">
                            <label class="control-label" for="inputDefault">{{{ trans('main.description') }}}</label>
                            <textarea class="form-control" rows="7" name="description">{{{ $part->description or '' }}}</textarea>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div data-plugin-spinner data-plugin-options='{ "value":0, "min": 0, "max": 100 }'>
                                <input type="number" value="{{{ $part->number or '' }}}" class="spinner-input form-control" maxlength="3">
                            </div>
                        </div>
